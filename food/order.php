...
if($order_id!=NULL){
    $qry = mysqli_query($conn , "INSERT INTO orders (customer_id , order_id,qty,customer_address_id,product_id,approval) VALUES('$arParams->customer_id','$order_id','$arParams->qty','$arParams->cust_adrs_id','$arParams->product_id','0')");
    if($qry){
        $response['data']['order_id'] = "Your order id is ".$order_id;
        $qry = mysqli_query($conn , "UPDATE products SET qty = qty-1 WHERE id='$arParams->product_id'");
        if($qry){
        echo json_encode(cmnresponse(TRUE,$response));
        }
    }
