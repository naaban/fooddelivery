...
$qry = mysqli_query($conn , "SELECT ord.order_id,ord.total_amt,pro.id,adm.id,ord.qty,ord.customer_address_id,creg.id,ord.customer_id,pro.name AS product_name,pro.description,pro.price,pro.image,creg.name AS cust_name,creg.mobile,creg.email,cust.street,cust.door_no,cust.city,cust.state,cust.pincode FROM admin_reg AS adm JOIN products AS pro ON pro.admin_id = adm.id JOIN orders AS ord ON ord.product_id = pro.id JOIN customer_address AS cust ON cust.id = ord.customer_address_id JOIN customer_reg AS creg ON creg.id = ord.customer_id WHERE adm.id = '$arParams->admin_id'");
